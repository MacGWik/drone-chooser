...
class Fc extends MY_Controller {
	...
	{
		$this->checkLogin('admin/fc/index');
		if($this->session->userdata('class') == 'admin')
		{
			$data['main_content'] = "admin/fc/list";
	...
	{
		$this->checkLogin('admin/fc/create');
		if($this->session->userdata('class') == 'admin')
		{

			$post = $this->PopulatePost();
			if(isset($post['submit'])){
				$this->fcmodel->Insert($post);
				redirect('admin/fc');
			}
			$data['datafcsoftware'] = $this->fcsoftwaremodel->GetAllData();
			$data['datafcmountoption'] = $this->fcmountoptionmodel->GetAllData();
			$data['dataescsoftware'] = $this->escsoftwaremodel->GetAllData();
			$data['main_content'] = "admin/fc/create";
			$this->load->view('templates/default',$data);
		}
		else
		{
	...
	public function edit($id){
		$this->checkLogin('admin/fc/edit/'.$id);
		if($this->session->userdata('class') == 'admin')
		{
			$data['datafc'] = $this->fcmodel->GetDataByID($id);
			
			$post = $this->PopulatePost();
			if(isset($post['submit'])){
				$post['id'] = $id;
				$this->fcmodel->Update($post);
				redirect('admin/fc');
			}
			$data['datafcsoftware'] = $this->fcsoftwaremodel->GetAllData();
			$data['datafcmountoption'] = $this->fcmountoptionmodel->GetAllData();
			$data['dataescsoftware'] = $this->escsoftwaremodel->GetAllData();
			$data['main_content'] = "admin/fc/edit";
			$this->load->view('templates/default',$data);
		}
		else
		{
