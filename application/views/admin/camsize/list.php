		<div id="page-wrapper">
            <div class="row">
                <div class="col-lg-7" style="padding-right: 0px;">
                    <h1 class="page-header">FPV Camera Size</h1>
                </div>
                <div class="col-lg-5" style="padding-left: 0px; padding-top: 0px; text-align: right;">
                    <h1 class="page-header" style="margin-top: 39px;">
                        <a href="<?= base_url() ?>admin/camsize/create" class="btn btn-primary">Create FPV Camera Size</a>
                    </h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
...
<script>
	...
	function CreatedataTbl(){ //alert(jmlhari);
		...
		var tr = $('<table id="dataTbl" class="order-column hover"></table>')
			...
			.append($("<th ></th>").append("FPV Camera Size"))
		...
		dataTbl = $('#dataTbl').dataTable({
	        ...
	        "bProcessing": true,
	        "bServerSide": true,
	        "sAjaxSource": site_url+'datasource/camsize',
	        "aoColumnDefs": [
		        {
	...
	function resetCallBack()
	{
		$(".btnDelete").click(function(e){
			...
			$.displayConfirm("Are you sure to delete this data?",function(){
				// $(".loading").show();
				$.post("<?php echo site_url('admin/camsize/delete'); ?>",
				{ 
					id:data 
				},
					function(data){
